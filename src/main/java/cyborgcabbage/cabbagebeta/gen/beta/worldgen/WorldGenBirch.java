...
public class WorldGenBirch extends WorldGenerator {
	private final int height;

	public WorldGenBirch(int height) {
		this.height = height;
	}
				...
				if((state.isOf(Blocks.GRASS_BLOCK) || state.isOf(Blocks.DIRT)) && y < height - i6 - 1) {
					...
					for(i16 = 0; i16 < i6; ++i16) {
						...
						if(state2.isAir() || state2.isIn(BlockTags.LEAVES)) {
							...
							Blocks.BIRCH_LOG.getDefaultState().updateNeighbors(world, new BlockPos(x, y + i16, z), Block.NOTIFY_LISTENERS);
						}
					}
